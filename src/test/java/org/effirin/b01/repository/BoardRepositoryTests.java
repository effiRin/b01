package org.effirin.b01.repository;
...
public class BoardRepositoryTests {
    ...
    public void testReadWithImage() {
        ...
        //실제 첨부가 있는 번호를 테스트
        Optional<Board> result = boardRepository.findById(2L);   // 2번 게시물을 읽어옴

        Board board = result.orElseThrow();

        log.info(board);

        // Lazy라서 처음엔 board 테이블한 select 가져옴. 그런데...
        //getImgSet 때문에 @Tranasctional이 필요
    ...
    }// 이 코드를 실행하면 자동으로 join도 되고, 이미지도 들어가있음을 볼 수 있음


    @Transactional
    @Commit
    @Test
    public void testUpdateWithImage() {  // 이미지 교체
        ...
        board.clearImgSet();
        // elementCollection 특징 - update가 없어서 이미지 지우고 이미지 새로 insert
